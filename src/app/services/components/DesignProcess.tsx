      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[48px] font-bold 
            mb-10 sm:mb-16 md:mb-20 max-w-full sm:max-w-[510px]"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {processSteps.map((step, index) => (
              <div 
                ...
                className={`${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'}
                  p-6 sm:p-8 md:p-10 rounded-lg flex flex-col items-center justify-between 
                  h-64 sm:h-72 md:h-80 transition-all duration-300 ease-in-out 
                  hover:-translate-y-4 hover:shadow-lg cursor-pointer`}
              >
            ...
            <div 
              className={`${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'}
                p-6 sm:p-8 md:p-10 rounded-lg flex items-center 
                h-64 sm:h-72 md:h-80 transition-all duration-300 ease-in-out 
                hover:-translate-y-4 hover:shadow-lg cursor-pointer`}
            >
              <p className="text-base sm:text-lg leading-relaxed">
                As designs move from our screens to yours we take care of everything in between. 
                Pre and post launch we do user testings and use metrix to ensure the output is right.
              </p>
            </div>
          </div>
        </div>
